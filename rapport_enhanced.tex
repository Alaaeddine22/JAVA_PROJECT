...
Dans le cadre du module NoSQL de notre formation en Génie Informatique à l'EMSI, nous avons développé une \textbf{plateforme de streaming de données en temps réel}. Ce projet vise à démontrer l'intégration de plusieurs technologies de bases de données NoSQL et relationnelles pour gérer des flux de données massifs.

La plateforme \textbf{Streaming Platform} est conçue pour simuler un environnement de messagerie distribuée similaire à Apache Kafka, où des \textbf{Producteurs} (Producers) envoient des messages vers des \textbf{Topics}, qui sont ensuite consommés par des \textbf{Consommateurs} (Consumers). Cette architecture permet de gérer efficacement des millions de messages avec une latence minimale.

\subsection{Contexte Technologique}
Les systèmes de streaming de données sont devenus incontournables dans l'écosystème Big Data moderne:
\begin{itemize}
    \item \textbf{Apache Kafka} traite plus de 7 trillions de messages par jour chez LinkedIn
    \item \textbf{Amazon Kinesis} gère les flux de données AWS à l'échelle mondiale
    \item \textbf{Apache Pulsar} offre une architecture multi-tenant pour le cloud
\end{itemize}

Notre projet s'inspire de ces systèmes tout en offrant une approche pédagogique pour comprendre les mécanismes sous-jacents.

\section{Objectifs du Système de Streaming en Temps Réel}

\subsection{Objectifs Techniques}
\begin{enumerate}
    \item \textbf{Architecture Multi-Tiers}: Concevoir une architecture N-Tier avec séparation claire des responsabilités (Présentation, Logique Métier, Données).
    \item \textbf{Persistance Polyglotte}: Intégrer trois bases de données hétérogènes:
    \begin{itemize}
        \item H2 Database pour l'ingestion rapide (Hot Storage)
        \item MySQL pour l'analyse structurée et les métadonnées
        \item Neo4j pour la modélisation des relations complexes
    \end{itemize}
    \item \textbf{Synchronisation Temps Réel}: Maintenir la cohérence des données entre les trois bases avec des mécanismes de synchronisation automatiques.
    \item \textbf{API REST Complète}: Exposer toutes les fonctionnalités via une API RESTful avec opérations CRUD.
    \item \textbf{Interface Utilisateur Moderne}: Créer un dashboard interactif avec visualisations dynamiques.
\end{enumerate}

\subsection{Objectifs Pédagogiques}
\begin{itemize}
    \item Maîtriser le langage Cypher pour les requêtes sur graphes Neo4j
    \item Comprendre les patterns de synchronisation inter-bases
    \item Implémenter une architecture orientée services avec Node.js
    \item Développer des interfaces utilisateur réactives avec JavaScript ES6+
\end{itemize}

\section{Problématique: Gestion de la Vélocité et de la Complexité des Données}

\subsection{Les Défis du Streaming de Données}
...
\begin{enumerate}
    ...
    \item \textbf{Variété}: Comment gérer des données de structures différentes (relationnelles et graphes)?
    \item \textbf{Véracité}: Comment assurer la cohérence des données entre les différentes bases?
\end{enumerate}

\subsection{Notre Approche}
Pour répondre à ces défis, notre architecture utilise:
\begin{itemize}
    \item \textbf{H2 comme buffer d'ingestion}: Base embarquée ultra-rapide pour absorber les pics de charge
    \item \textbf{MySQL pour la persistance}: Stockage ACID pour les données critiques
    \item \textbf{Neo4j pour les relations}: Modélisation naturelle des liens Producer-Topic-Consumer
    \item \textbf{Services de synchronisation}: Jobs périodiques pour maintenir la cohérence
\end{itemize}

\section{Organisation du Rapport}
Ce rapport est structuré en 9 chapitres:
\begin{itemize}
    \item \textbf{Chapitre 1}: Introduction et problématique
    \item \textbf{Chapitre 2}: Architecture système et réseau
    \item \textbf{Chapitre 3}: Stratégie de persistance multi-bases
    \item \textbf{Chapitre 4}: Services de synchronisation et flux de données
    \item \textbf{Chapitre 5}: Gestion du cycle de vie des données
    \item \textbf{Chapitre 6}: Interface utilisateur et monitoring
    \item \textbf{Chapitre 7}: Analyse critique et sécurité
    \item \textbf{Chapitre 8}: Conclusion et perspectives
    \item \textbf{Chapitre 9}: Annexes techniques
\end{itemize}
...
\subsection{Diagramme de Séquence: Flux d'Ingestion et Synchronisation}
La figure suivante illustre le flux complet depuis la réception d'un message jusqu'à sa synchronisation:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{diagrams/sequence.png}
    \caption{Diagramme de Séquence: Flux d'Ingestion et Synchronisation}
    \label{fig:sequence}
\end{figure}

Le diagramme de séquence (\ref{fig:sequence}) montre le processus complet:
\begin{enumerate}
    \item Le Producer envoie un message (Topic + Payload) via SocketServer
    \item Le ClientWorker persiste le message dans H2 Database
    \item Un accusé de réception (Ack) est renvoyé au Producer
    \item La synchronisation périodique (toutes les 5s) transfère les données vers MySQL et Neo4j
    \item Les services de synchronisation maintiennent la cohérence entre les bases
\end{enumerate}

\subsection{Diagramme de Classes: Structure du Projet}
La figure suivante présente l'architecture des classes principales du système:

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{diagrams/class.png}
    \caption{Diagramme de Classes: Structure du Projet Java}
    \label{fig:class}
\end{figure}

Le diagramme de classes (\ref{fig:class}) illustre:
\begin{itemize}
    \item \textbf{SocketServer}: Gère les connexions TCP et le pool de threads
    \item \textbf{ClientWorker}: Traite chaque connexion client individuellement
    \item \textbf{DatabaseManager}: Abstrait l'accès à H2 Database
    \item \textbf{Services de Synchronisation}: MySQLSyncService et Neo4jSyncService avec leurs managers respectifs
    \item \textbf{ArchiveService}: Gère le cycle de vie des données
\end{itemize}

\section{Stack Technologique}

Notre plateforme repose sur un stack technologique moderne et performant, organisé en quatre couches distinctes.
...
\begin{figure}[H]
    ...
    \begin{tcolorbox}[
        ...
        colback=teal!10,
        colframe=teal,
        title={\textbf{Couche Présentation - Frontend}},
        fonttitle=\bfseries,
        arc=2mm,
        boxrule=1pt,
        left=2mm,
    ...
    \centering\small
    \begin{tabular}{cccc}
        \includegraphics[height=1.8cm]{logos/html_css_js_logo.png} &
        \includegraphics[height=1.8cm]{logos/chartjs_logo.png} &
        \raisebox{0.4cm}{\textbf{Glassmorphism}} &
        \raisebox{0.4cm}{\textbf{Fetch API}} \\[0.2cm]
        \scriptsize HTML5/CSS3/JS & \scriptsize Chart.js & \scriptsize Design & \scriptsize REST
    \end{tabular}
    \end{tcolorbox}
    ...
    \begin{tcolorbox}[
        ...
        colback=orange!10,
        colframe=orange,
        title={\textbf{Couche API - Backend}},
        fonttitle=\bfseries,
        arc=2mm,
        boxrule=1pt,
        left=2mm,
    ...
    \centering\small
    \begin{tabular}{ccc}
        \includegraphics[height=1.8cm]{logos/nodejs_logo.png} &
        \raisebox{0.4cm}{\textbf{Express.js}} &
        \raisebox{0.4cm}{\textbf{neo4j-driver}} \\[0.2cm]
        \scriptsize Node.js Runtime & \scriptsize Framework REST & \scriptsize Driver Neo4j
    \end{tabular}
    \end{tcolorbox}
    ...
    \begin{tcolorbox}[
        ...
        colback=emsiblue!10,
        colframe=emsiblue,
        title={\textbf{Couche Logique Métier}},
        fonttitle=\bfseries,
        arc=2mm,
        boxrule=1pt,
        left=2mm,
    ...
    \centering\small
    \begin{tabular}{ccc}
        \includegraphics[height=1.8cm]{logos/java_logo.png} &
        \includegraphics[height=1.8cm]{logos/maven_logo.png} &
        \raisebox{0.4cm}{\textbf{ThreadPool}} \\[0.2cm]
        \scriptsize Java 17 LTS & \scriptsize Maven 3.9 & \scriptsize Concurrence
    \end{tabular}
    \end{tcolorbox}
    ...
    \begin{tcolorbox}[
        enhanced,
        colback=codepurple!10,
        colframe=codepurple,
        title={\textbf{Couche Données - Bases de Données}},
        fonttitle=\bfseries,
        arc=2mm,
        boxrule=1pt,
        left=2mm,
    ...
    \centering\small
    \begin{tabular}{ccc}
        \includegraphics[height=1.8cm]{logos/h2_logo.png} &
        \includegraphics[height=1.8cm]{logos/mysql_logo.png} &
        \includegraphics[height=1.8cm]{logos/neo4j_logo.png} \\[0.2cm]
        \scriptsize H2 (Hot Storage) & \scriptsize MySQL (XAMPP) & \scriptsize Neo4j Desktop
    \end{tabular}
    \end{tcolorbox}
    
    \end{tcolorbox}
    \caption{Architecture Technologique Complète de la Plateforme}
    \label{fig:techstack}
\end{figure}
...
\section{Processus de Synchronisation Multi-Bases}

Notre plateforme implémente un système de synchronisation complexe entre trois bases de données hétérogènes. Le diagramme de séquence (Figure \ref{fig:sequence}) illustre clairement ce flux:

\subsection{Synchronisation Périodique (Every 5s)}
Comme montré dans le diagramme, un job de synchronisation s'exécute toutes les 5 secondes:
\begin{enumerate}
    \item \textbf{Query recent messages/counts}: Extraction des données récentes de H2
    \item \textbf{UPSERT stats into ip\_topic\_stats}: Mise à jour des statistiques dans MySQL
    \item \textbf{Query unique Producers/Topics}: Extraction des entités uniques
    \item \textbf{Merge (Nodes) \& CREATE (Relationships)}: Mise à jour du graphe Neo4j
\end{enumerate}
...
\begin{itemize}
    \item[\textcolor{red}{!}] Les mots de passe sont actuellement en dur dans le code
...
\subsection{Diagramme de Séquence Complet}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{diagrams/sequence.png}
...
Description du flux:
\begin{itemize}
    \item \textbf{Producer}: Émetteur de messages vers la plateforme
    \item \textbf{SocketServer/ClientWorker}: Réception et traitement initial
    \item \textbf{H2 Database}: Stockage temporaire haute performance
    \item \textbf{MySQLSyncService}: Synchronisation vers MySQL pour l'analyse
    \item \textbf{Neo4jSyncService}: Synchronisation vers Neo4j pour les relations
    \item \textbf{Synchronisation Périodique}: Processus automatique toutes les 5 secondes
\end{itemize}

\subsection{Diagramme de Classes Détaillé}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{diagrams/class.png}
    \caption{Diagramme de Classes: Architecture Logicielle}
    \label{fig:appendix-class}
\end{figure}

Structure des packages:
\begin{itemize}
    \item \textbf{streamingplatform}: Package racine
    \item \textbf{net}: Gestion réseau (SocketServer, ClientWorker)
    \item \textbf{persistence}: Accès aux bases (DatabaseManager)
    \item \textbf{service}: Logique métier et synchronisation
    \item \textbf{ArchiveService}: Gestion du cycle de vie
    \item \textbf{MySQLSyncService/Neo4jSyncService}: Services de synchronisation
\end{itemize}
